/***************************************************************************//**
* \file smpt_ll_serial_port_macos.c
...
bool smpt_open_serial_port_macos(int *fd, const char port_name[],
    ...
    *fd = open(port_name, O_RDWR | O_NOCTTY | O_NONBLOCK);
    if (*fd > 0)
    {
        ...
        if (tcsetattr(*fd, TCSANOW, &tio) != 0)
        {
            if (debug_error)
            {
                smpt_error("smpt_open_serial_port_macos(): Could not set serial attributes (err: %d)",
                           errno);
                result = false;
            }
        }
            ...
            (cfsetispeed(&tio, baud_rate) < 0))
        {
            if (debug_error)
            {
                smpt_error("smpt_open_serial_port_macos(): Could not set baud rate");
            }
            result = false;
        }
...
bool smpt_write_to_serial_port_macos(int *const fd, const uint8_t output_data[],
                                     uint32_t output_data_length)
{
    ...
    {
        smpt_error("smpt_write_to_serial_port_macos(): Could not write all bytes \
                   (written: %d, length: %d)",
...
bool smpt_read_from_serial_port_macos(uint32_t *bytes_read, int fd,
                                      uint8_t input_data[], uint32_t input_data_length)
{
    ...
    if (fd > 0)
    {
        ...
        else
        {
            ...
            {
                smpt_error("smpt_read_from_serial_port_macos(): Read error (%s)", strerror(errno));
                result = false;
            }
            else
            {
...
bool smpt_close_serial_port_macos(int fd)
{
...
int set_flowcontrol(int fd, int control)
{
    ...
    {
        smpt_error("smpt_serial_port_macos.c: error from tggetattr");
        return -1;
    }
    ...
    {
        smpt_error("smpt_serial_port_macos.c: error setting term attributes");
        return -1;
    }
    return 0;
}
